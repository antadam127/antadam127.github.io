...
// SET FILTERS
// Sets to Featured
if (showFeatMode && startOnFeatMode && includeFeat) {
    // $('#filters').append(`<a id="featured-filter" href="#" data-filter=".featured">Featured</a>`); // Adds Featured
    $(document).ready(function () {
        setTimeout(function () {
            [...document.querySelectorAll('#filters a')][1].click();
            // document.getElementById('featured-filter').click();
        }, 50);
    });
}
// Set Filters
    ...
    $('#portfolio-grid').append(`
        <div id="itemAdded_${i + 1}" class="item col-sm-6 col-md-4 col-lg-4 mb-4">
            <a href="portfolio/index-page.html?item=${e.id}" class="item-wrap fancybox" style="box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px;">
                ...
                <img class="img-fluid" src="portfolio/images/${e.id}.jpg" style="aspect-ratio: 1.27" />
            </a>
        </div>
    `);
