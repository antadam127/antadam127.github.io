...
function createCustomGeocoder() {
  ...
  $("body").append(el);
  $(".mapboxgl-ctrl-geocoder").css("min-width", "100%");
}
...
function createSearchbar() {
  ...
  const coordinatesGeocoder = function (query) {
    if (config.searchbarOptions.allowReverseGeocode) {
      ...
      if (!matches) {
        switchCoords = true;
        matches = query.match(/^[ ]*(?:Lng: )?(-?\d+\.?\d*)[, ]+(?:Lat: )?(-?\d+\.?\d*)[ ]*$/i);
      }
  ...
  const newGeocoder = new MapboxGeocoder({
    ...
    flyTo: config.searchbarOptions.instantFly
      ? { duration: 0 }
      : config.buildingOptions.requestGeocodeZoomLevel
      ? { zoom: config.buildingOptions.requestGeocodeZoomLevel }
      : true, // Fly or Jump
